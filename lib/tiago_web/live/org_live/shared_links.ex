defmodule TiagoWeb.OrgLive.SharedLinks do
  ...
  def handle_event("create_link", %{"party_id" => pid, "label" => label}, socket) do
    ...
    {:ok, _} =
      Sharing.create_shared_link(%{
        party_id: String.to_integer(pid),
    ...
    {:noreply,
     ...
     |> assign(links: Sharing.list_links_for_org(org_id))}
  end
  ...
  def handle_event("deactivate", %{"id" => id}, socket) do
    ...
    {:noreply,
     socket
     |> put_flash(:info, "Link deactivated")
     |> assign(links: Sharing.list_links_for_org(socket.assigns.current_org.id))}
  end
        ...
        <form phx-submit="create_link" class="flex gap-2 items-end">
          ...
          <div class="flex-1">
            ...
            <input
              type="text"
              name="label"
              placeholder="e.g. Shared with ABC Ltd"
              class="w-full rounded-lg border-gray-300 text-sm"
            />
          </div>
          <button
            type="submit"
            class="bg-blue-600 text-white rounded-lg px-4 py-2 text-sm font-medium"
          >
            Create
          </button>
        </form>
              ...
              <%= for l <- @links do %>
                <tr>
                  ...
                  <td class="px-4 py-3 text-sm">
                    <code class="text-xs bg-gray-100 px-2 py-1 rounded">/shared/{l.token}</code>
                  </td>
                  <td class="px-4 py-3 text-sm">
                    <span class={"px-2 py-1 text-xs rounded-full #{if l.active, do: "bg-green-100 text-green-800", else: "bg-red-100 text-red-800"}"}>
                      {if l.active, do: "Active", else: "Inactive"}
                    </span>
                  </td>
