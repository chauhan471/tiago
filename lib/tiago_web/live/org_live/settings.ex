defmodule TiagoWeb.OrgLive.Settings do
  ...
  def handle_event("invite", %{"email" => email, "role" => role}, socket) do
    ...
    cond do
      ...
      Organizations.member?(user.id, org_id) ->
        {:noreply, put_flash(socket, :error, "Already a member")}
      ...
      true ->
        ...
        {:noreply,
         socket
         |> put_flash(:info, "#{email} added as #{role}")
         |> assign(members: Organizations.list_members(org_id))}
    end
  end
  ...
  def handle_event("remove", %{"user-id" => uid}, socket) do
    ...
    {:noreply,
     ...
     |> assign(members: Organizations.list_members(socket.assigns.current_org.id))}
  end
            ...
            <%= for m <- @members do %>
              <tr>
                ...
                <td class="py-3 text-sm">
                  <span class={"px-2 py-1 text-xs rounded-full #{if m.role == :admin, do: "bg-purple-100 text-purple-800", else: "bg-blue-100 text-blue-800"}"}>
                    {m.role}
                  </span>
                </td>
