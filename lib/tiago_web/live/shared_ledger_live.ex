defmodule TiagoWeb.SharedLedgerLive do
  ...
  def mount(%{"token" => token}, _session, socket) do
    case Sharing.get_active_link_by_token(token) do
      nil ->
        {:ok, socket |> assign(page_title: "Link Not Found", error: true, ledger: nil, org_name: nil)}

      link ->
        ledger = Ledger.party_ledger(link.party_id)
        {:ok, assign(socket, page_title: "#{ledger.party.name} — Shared Ledger",
                     error: false, ledger: ledger, org_name: link.organization.name, token: token)}
    end
  end
    ...
    <div class="max-w-7xl mx-auto px-4 py-8">
      <%= if @error do %>
        <div class="text-center py-20">
          ...
          <p class="text-gray-500">This shared ledger link is invalid, expired, or has been deactivated.</p>
        </div>
      <% else %>
        ...
        <div class="bg-white shadow rounded-lg overflow-hidden">
          ...
          <% else %>
            <table class="min-w-full divide-y divide-gray-200">
              ...
              <tfoot class="bg-gray-100"><tr class="font-bold">
