  ...
  def render(assigns) do
    ...
    <div class="max-w-7xl mx-auto px-4 py-8">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        <.link
          navigate={~p"/parties?type=customer"}
        ...
        <.link
          navigate={~p"/parties?type=supplier"}
          class="bg-white shadow rounded-lg p-6 hover:shadow-md transition"
        >
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-10">
        <.link
          ...
          class="flex items-center justify-center gap-2 bg-blue-600 text-white rounded-lg px-6 py-3 font-medium hover:bg-blue-700 transition"
        >
          + Add Party
        </.link>
        <.link
          navigate={~p"/uploads"}
          class="flex items-center justify-center gap-2 bg-green-600 text-white rounded-lg px-6 py-3 font-medium hover:bg-green-700 transition"
        >
          📤 Upload Files
        </.link>
        <.link
          navigate={~p"/parties"}
          class="flex items-center justify-center gap-2 bg-purple-600 text-white rounded-lg px-6 py-3 font-medium hover:bg-purple-700 transition"
        >
          📒 View Ledgers
        </.link>
      </div>
      ...
      <div class="bg-white shadow rounded-lg overflow-hidden">
        <div class="px-6 py-4 border-b">
          <h2 class="text-lg font-semibold">Recent Journals</h2>
        </div>
        <%= if @recent_journals == [] do %>
          <div class="px-6 py-12 text-center text-gray-500">
            No journals yet. Upload files to get started.
          </div>
        <% else %>
                ...
                <tr class="hover:bg-gray-50">
                  ...
                  <td class="px-6 py-4 text-sm">
                    <span class="px-2 py-1 text-xs rounded-full bg-gray-100">
                      {Map.get(entry, :transaction_type, "—")}
                    </span>
                  </td>
                  <td class="px-6 py-4 text-sm text-gray-500">
                    {if j.party, do: j.party.name, else: "—"}
                  </td>
                </tr>
              <% end %>
            </tbody>
